...
const form = useForm({
    ...
    email: props.head.email,
    password: '',
    password_confirmation: '',
    role: props.head.role ?? '',
    institute_id: props.head.institute_id?.toString() ?? '',
    program_id: props.head.program_id?.toString() ?? '',
})
...
const submit = () => {
    form.put(`/admin/heads/${props.head.id}`)
}
</script>

<template>
    <Head :title="`Edit ${head.name}`" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-1 flex-col gap-6">
            <PageHeader title="Edit Head Account" :subtitle="head.name" />
                ...
                <CardContent>
                    <form @submit.prevent="submit" class="space-y-4 p-1 pt-4">
                        ...
                        <div class="flex items-center gap-4">
                            <Button type="submit" :disabled="form.processing">Update Head</Button>
                            <Link :href="`/admin/heads/${head.id}`">
