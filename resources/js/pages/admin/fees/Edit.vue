...
const years = (props.fee.required_years ?? ['all']).includes('all') ? ['1', '2', '3', '4'] : props.fee.required_years ?? [];
...
const form = useForm({
    ...
    academic_term_id: props.fee.academic_term_id ? String(props.fee.academic_term_id) : (termDefault ? String(termDefault.id) : ''),
    required_years: [...years] as string[],
    due_date: props.fee.due_date?.slice(0, 10) || '',
});
...
const submit = () => {
    ...
    form.put(route('admin.fees.update', { fee: props.fee.id }));
};
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-1 flex-col gap-6">
            <PageHeader title="Edit Fee" subtitle="Update this fee. Changes sync to assigned students when posted." />
                ...
                <CardContent class="p-6">
                    <form @submit.prevent="submit" class="flex flex-col gap-5">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-2">
                            <a :href="`/admin/fees/${fee.id}`" class="rounded-xl px-4 py-2.5 text-sm font-medium text-gray-500 hover:bg-gray-100">
                                Cancel
                            </a>
