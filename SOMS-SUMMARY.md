...
- **Crash fix `f423ea1`**: recording still failed in production — `Cannot read properties of null (reading 'account_name')` from the StudentDetail chunk. Root cause: `feeAccount` was a **function**, so `v-if="feeAccount"` tested the function reference (always truthy) → the null branch rendered `feeAccount().account_name`. Now a `computed()` (auto-unwrapped in template) so `v-if` correctly falls through to the amber hint panel and **Record Cash stays enabled with no account**. Deployed + verified (new `StudentDetail-BaHscm0W.js` served; siblings 200).
- **Follow-up (same day, `6aa04e8`)**: the real cause of officers seeing an empty `/obligations` page was the **Outstanding tab listing heads** (SSC/ISC/SRO) — enrolled staff appeared in the list (no obligations in the officer's scope → nothing to select → Record Cash disabled). **`EligibilityService::studentIds` now excludes staff**: any user holding a head/officer/adviser/super_admin pivot role is dropped (`excludeStaff` → `whereDoesntHave('organizations', role in officerRoles+super_admin)`) in both enrollment and legacy paths. Fixes Outstanding, dynamic fee obligations, penalties, dashboard student counts and event QR requirements at once; officers/heads still get org notifications via the explicit officer union. Two `AdminPaymentsTest` regressions added (enrolled heads excluded from outstanding; walk-in cash with **no** PaymentAccount recorded end-to-end with `processed_by` + receipt). Suite: **325 passed (1493 assertions)**; SSH-deployed (`deploy.sh`, siblings verified 200).

### Deploy verification
- `curl -sI https://luxmap.devokss.online/login` 200 (new `app-80Tj6Zfd.js`), `labsync`/`hulagway` 200, `curl /build/manifest.json` shows `Index-D6mkC2nI.js` with `Delete All`, `Show-Bnzqx-gf.js` with `Processed by` + `Centavo`. PWA `2e67699` pushed to `DevOkss/LUXMAP-PWA:main` (Vercel auto). Follow-ups `6aa04e8` (Outstanding students-only eligibility fix) and `f423ea1` (StudentDetail null-account crash fix) SSH-deployed same day; all three domains re-verified 200.
